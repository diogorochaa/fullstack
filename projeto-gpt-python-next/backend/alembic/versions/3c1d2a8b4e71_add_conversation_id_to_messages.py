...
def upgrade() -> None:
    op.execute(
        sa.text("ALTER TABLE messages ADD COLUMN IF NOT EXISTS conversation_id UUID")
    )

    bind = op.get_bind()
    user_ids = bind.execute(
        sa.text("SELECT DISTINCT user_id FROM messages WHERE conversation_id IS NULL AND user_id IS NOT NULL")
    ).scalars().all()
